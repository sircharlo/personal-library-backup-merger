...
function spec(): FixtureSpec {
  return {
    deviceName: 'Untidy',
    rows: {
      Location: [R.bibleLoc(1, 1, 1), R.bibleLoc(2, 1, 2), R.docLoc(3, 3001)],
      UserMark: [
      ...
      Note: [
        ...
        R.note({ NoteId: 3, Guid: 'n3', LocationId: 3, Title: ' ', Content: '' }), // empty; only reference to location 3
...
describe('clean-ups on the merged result', () => {
  it('default clean-ups remove leftovers, re-attach notes and keep the output valid', async () => {
    ...
    expect(counts.Location).toBe(2); // unreferenced chapter 2 gone; chapter 3 kept (empty note still refers to it)
    expect(counts.UserMark).toBe(2);
    expect(counts.BlockRange).toBe(2);
    expect(counts.Note).toBe(4);
  ...
  it('removing empty notes also frees the location that only they referenced', async () => {
    const run = await runMerge([spec()], new Map(), undefined, ALL_ON);
    expect(run.cleanup?.emptyNotes).toBe(1);
    expect(run.cleanup?.unreferencedLocations).toBe(2);
    expect(run.result.counts.Note).toBe(3);
    expect(run.result.counts.Location).toBe(1);
    expect(run.result.counts.TagMap).toBe(1); // the empty note's tag assignment went with it
    await assertDatabaseSound(run);
    await assertReopenSanity(run);
  });
