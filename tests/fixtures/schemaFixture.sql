-- JW Library userData.db schema (schemaVersion 16 / grdb_migrations v14 variant, 18 tables, 29 indexes, 26 triggers).
-- Schema-only DDL extracted via sqlite3 userData.db .schema from a real backup. Contains ZERO personal data.
-- Used by tests/fixtures/builders.ts to build tiny synthetic .jwlibrary fixtures at test time.
-- NOTE: the merge engine never uses this file at runtime; it always extracts DDL live from the uploaded sources.
-- NOTE: the iPhone/iPad (grdb v16) variant additionally has TR_Update_LastModified_{Insert,Update,Delete}_InputField
--       triggers; builders.ts appends those on demand.
...
CREATE TRIGGER TR_Update_LastModified_Delete_IndependentMedia 
	...
	BEGIN 
		UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); 
	END;
CREATE TRIGGER TR_Update_LastModified_Insert_PlaylistItem
	INSERT ON PlaylistItem 
	BEGIN 
		UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); 
	END;
CREATE TRIGGER TR_Update_LastModified_Update_PlaylistItem 
	UPDATE ON PlaylistItem 
	BEGIN 
		UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); 
	END;
CREATE TRIGGER TR_Update_LastModified_Delete_PlaylistItem 
	DELETE ON PlaylistItem 
	BEGIN 
		UPDATE LastModified SET LastModified = strftime('%Y-%m-%dT%H:%M:%SZ', 'now'); 
	END;
